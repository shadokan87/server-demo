...
export const AilogwDefaultOptions = {
    log: false,
    activate: true,
    twilio: {
    ...
    polling: {
        delay: process.env["AILOGW_POLL_DELAY"] != undefined ? process.env["AILOGW_POLL_DELAY"] : "10:minutes",
        tailAmount: 10
    }
};
/**
...
export class Ailogw {
    ...
    #intervalId = -1;
    #fragola;
    #internalLog = process.env["AILOGW_LOG"] != undefined;
    #twilio;
    #voiceProvider;
    /**
     ...
     * ```
     */
    async sendSms(numberFrom, customContent) {
     ...
     */
    createAlertDiagnostic(params, logs) {
        ...
        return async () => {
            ...
            try {
                const watcherAgent = this.#fragola.agent({
                    ...
                    instructions: `# Log Analysis Instructions
...
**Remember: The topErrors[0] element is the most important - it must contain the actual critical issue that justifies the critical status. A message saying "Database completely unavailable" with INFO level is still critical and MUST be at index 0.**`,
     ...
     * ```
     */
    convertDelayToMs(delay) {
        if (typeof delay === 'number') {
            return delay;
        }
        const [valueStr, unit] = delay.split(':');
